...
parser=argparse.ArgumentParser(description="\"Mirrors\" a sample")
...
parser.add_argument("mirror_point",type=int,help="The point of the sample at which to mirror * 255")
...
if sample_rate<2000:
    ...
    if point>len(file_orig)-1:
        print("Error: Length out of range! Must be " + str(len(file_orig)//255) + " or below.")
    else:
        file_finished=list(file_orig[:point])+list(reversed(file_orig[:point]))
